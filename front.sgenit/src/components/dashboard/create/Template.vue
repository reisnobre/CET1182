<template lang="html">
  <div class="subPage" id="createTemplate">
    <form @submit.prevent="createTemplate()">
      ...
      <b-field>
        <b-upload v-model="files" accept="image/*">
          <span class="button is-primary" required>
            <b-icon class="__white" icon="file_upload"></b-icon>
      ...
      <span class="__small __opaque" style="display: block; margin-bottom: 1rem; margin-top: -0.5rem">Dimensões 1500x160</span>
      <img :src="dataURL" alt="" v-if="dataURL">
      <button type="submit" name="button" :class="['']">Cadastrar</button>
...
<script>
import { templates, footers } from '../../../routes/api'
  ...
  methods: {
    ...
    */
    createTemplate () {
      ...
      this.$http.post(templates, templateData, {
        headers: getHeader()
      }).then(response => {
